...
> **v0.1.28 道具图鉴页视觉统一：§3.7 `ItemDexView` 重构为与主菜单 / 事件遭遇页同族的样式 —— 顶栏一行（左返回金色胶囊 / 中「道具图鉴 ITEM DEX」白描边横幅 + 副提示胶囊 / 右统计信息卡复用 `.rogue-info`）；筛选改四档胶囊（新增 `.dex-filter`：未选中金色、选中蓝芯 + 黄晕）；卡片改白底 + 深蓝描边环 + 投影（新增 `.dex-card`，悬停环加深），类型改徽章 + 名称，穿脱下拉开复用 `.slot-action` / `.slot-delete` / `.starter-combo`；新增错峰入场动效（启动页参数基线，卡片仅前 8 张参与）；筛选切换后列表回顶、穿脱刷新保持滚动位置；消耗品使用入口表述同步为「主菜单中栏的道具详情框」**；
> **v0.1.29 道具图鉴改双栏版面：§3.7 `ItemDexView` 从全宽卡片列表改为「左信息框 + 右列表」双栏（45% / 55%，与商店同分栏）——左栏 `.dex-detail` 白底蓝环卡（上 46% 图片区：`/images/tool/` 与 `/images/portable Items/` 两级查找插图、缺图回退首字色块；下 54% 文字区：徽章 + 名称 / 拥有量状态 / 来源 / 效果说明 / 穿脱操作行），右栏条目行（`.dex-card` 改 `.dex-row` 行卡：徽章 + 名称 + 状态，悬停环加深并联动左栏）；穿脱操作移入左栏并改为就地刷新（统计卡 / 行状态 / 左框，不重建列表）；左栏骨架一次构建、悬停联动只改内容；主体双栏撑满筛选栏以下剩余高度（列表条目少时不塌缩）**；
> **v0.1.30 商店装备恢复随机上架（每次 3 件）：§3.6 `ShopView` 段改写 —— 装备分区每次进店从未拥有池（最多 77 件）中**随机抽 3 件**上架、不再全量列出；上架顺序仍为「基础价升序 → id」、售价仍按段通胀、买走即下架不补货；`RouteConfig` 恢复格位常量 `SHOP_EQUIPMENT_STOCK_SIZE = 3`（不随段变化），`ShopStock#allEquipmentEntries` 改回抽签版 `#equipmentEntries(segment, random, owned)`（`random` 重新生效、`null` 时兜底新建，池不足 3 件时全部上架）；§3.7 图鉴未拥有装备提示同步为「每次进店随机上架」措辞**；
> **v0.1.31 商店分区标题瘦身：§3.6 `ShopView` 货架两个分区标题由「消耗品 · 已解锁 N 件（解锁后一直有货，可重复购买）」/「装备 · 本次上架 N 件（每次进店随机进货，购买后可在主菜单中栏穿戴）」精简为「消耗品」「装备」两行纯文本，不再渲染件数与说明文案**；
> **v0.1.32 中栏信息框纸张质感：主菜单中栏简要信息框卡片由纯白底改为图鉴纸感 —— 柔和浅米黄纵向渐变底（#fdf9ee → #f9f2e0 → #f5ecd6）+ 中央微光 + 左下/右上两处极淡做旧晕 + 边缘泛黄做旧（多层 radial-gradient 叠加，不引入图片资源）；深蓝描边环与布局不变，投影调为暖褐色轻影（rgba(84, 68, 40, 0.32)）；文字 / 立绘 / 交互全不变**；
...
```
                ...
                └──────┬───────┘   「开始游戏」新档、「继续游戏」读档
                       ├────────► pokedex 宝可梦图鉴 / itemDex 道具图鉴（启动页入口，无存档时只读）
                       ▼
                ┌──────────────┐
                │ mainMenu 主菜单│ ← 新建档完成 / 读档完成
                └──────┬───────┘   「保存游戏」「读取存档」「返回主界面」
                       ├────────► itemDex 道具图鉴（94 件全量 + 装备穿脱）
                       ▼
                ┌──────────────┐
...
| --- | --- | --- | --- |
| 启动页 | 开始游戏（新档）/ 继续游戏（读档，无档置灰）/ 宝可梦图鉴 / 道具图鉴（只读） / 成就 / 设置 | 游戏流程系统 + 存档系统 | SAVE-01~03 |
...
| 道具图鉴（参考实现已落地） | 启动页「道具图鉴」（无存档 → 只读）/ 主菜单「道具图鉴」（可直接穿脱装备）进入：94 件全量目录（17 道具 + 77 装备）、四档筛选、左信息框（图片 / 信息 / 拥有量）+ 右列表、已拥有/穿戴标注、装备直接穿脱 | 肉鸽系统（`ShopStock.catalog()` 定价与「在售 / 不售卖」口径）+ 宝可梦系统（装备库 / 背包） | 见 §3.7 |
| 精灵详情页（参考实现已落地） | 主菜单点精灵名进入：立绘/信息/数值/技能 + 装备槽（穿戴/脱下）+ 道具使用卡（局外喂药 / 吃神奇糖果） | 宝可梦系统 + 肉鸽系统（装备库、`ItemUsageService`） | UI-03 相关 |
...
### 3.5 精灵详情页与装备穿戴（v0.1.10，参考实现已落地；v0.1.26 补道具使用）
...
- **道具使用卡**（v0.1.26 新增）：列出背包里**可在局外使用**的消耗品（`Item#usableOutsideBattle()`，
  即回复类 / 解除类 / 升级类；精灵球不在内），每行给出名称、效果说明、`已拥有 ×N` 与「使用」按钮。
  点击后把该件用在**当前详情的这只精灵**上，并把 `ItemUsageService#use` 返回的文案弹窗展示；
  **不满足条件的操作不消耗道具**（伤药对满 HP / 已倒下、解除药对无对应异常、神奇糖果对满级）。
  升级走 `BattleGrowthPort#boostLevel`，因此吃神奇糖果与战斗结算出的升级同规则（到级学招 / 进化）。
  用掉道具后本页即时重建，主菜单的背包计数随之刷新。
...
「神奇糖果」另有肉鸽「糖果补给」事件（`OptionType.CANDY`，见 §3.6）与商店两条获取途径。
...
>
> **v0.1.26**：特殊事件槽位新增一次性节点「**糖果补给**」（`OptionType.CANDY`，1 点行动点，一次性），
...
- **货架分两个分区**：标题仅「消耗品」「装备」两行纯文本（v0.1.31 起不再渲染件数与说明文案），
  先消耗品后装备；装备为未拥有池中随机上架的 3 件（购买后入库并从货架下架、不补货）；
- **消耗品逐段解锁、解锁后常驻**（v0.1.24；v0.1.26 增神奇糖果）—— 第 1 段只有普通精灵球 /
  伤药 / 5 种状态药共 7 件，之后每段再放开（第 2 段纪念球·好伤药·**神奇糖果** → 第 3 段超级球·狩猎球 →
  第 4 段竞赛球·高级球 → 第 5 段万灵药·贵重球）。解锁条件是「当前段 ≥ 解锁段」，故**已经解锁的
  消耗品在后续所有段都仍在货架上**，货架只增不减（7 → 10 → 12 → 14 → 16 件），不抽签、不重新洗牌；
- **装备不分段解锁、每次进店随机上架 3 件**（v0.1.21 起不分段 + v0.1.30 恢复随机抽签）——
  第 1 段起全部可买，但货架不再全量列出：`ShopStock` 每次生成货架时从「未拥有的装备池」
  （47 件对战道具 + 30 种树果）中**随机抽 `RouteConfig#SHOP_EQUIPMENT_STOCK_SIZE`（3）件**上架
  （池不足 3 件时全部上架），上架顺序为「基础价升序 → id」（与图鉴同口径）；购买后从货架下架、
  **买完不补货**，想碰其他装备需等下一次进店（商店为一次性节点，每次进店重抽）；
- **消耗品不设格位上限、装备固定 3 件**（v0.1.24 + v0.1.30）—— 旧的 `RouteConfig#shopConsumableStockSize`
  （3 → 6 件）与四个 `SHOP_CONSUMABLE_*` 常量已删除；「越往后越多」只剩**消耗品解锁数量**与
  **售价通胀**（`RouteConfig#shopPrice`，每段 +15%，见需求文档 §4.5）两处。装备恢复格位概念
  （v0.1.30）：`RouteConfig#SHOP_EQUIPMENT_STOCK_SIZE = 3`，不随段变化；
...
>
> **v0.1.26（新增神奇糖果）**：消耗品池新增「神奇糖果」（`i_rare_candy`，`ItemCategory.LEVEL_UP`，
> 基础价 100、**第 2 段起解锁**），因此消耗品解锁链变为第 1 段 7 件 → 第 2 段 10 件，
> 逐段货架件数为 **7 → 10 → 12 → 14 → 16**（只增不减的口径不变）。
> 除「战斗内投球 / 战斗中吃药」外，本件还多一条**局外**用法：精灵详情页的道具使用卡
> （见 §3.5）可把伤药 / 状态药 / 神奇糖果直接喂给精灵，商店页本身仍只负责买卖。
...
### 3.7 道具图鉴页（v0.1.20 新增、v0.1.22 补启动页入口、v0.1.29 改双栏版面，已落地）

**`ItemDexView`（道具图鉴）** —— 启动页「道具图鉴」或主菜单「道具图鉴」按钮进入，纯展示 + 装备穿脱：

- 构造参数 `ItemDexView(player, background, onBack)`（返回文案固定「返回主菜单」）与
  4 参重载 `ItemDexView(player, background, onBack, backLabel)`（启动页传「返回主界面」）；
  `createScene()` 依次渲染顶栏（左返回胶囊 / 中标题横幅 / 右统计信息卡）、筛选胶囊栏与
  主体双栏（样式与主菜单 / 事件遭遇页统一：地图背景 + 白底蓝环卡族 + 错峰入场动效），
  `onBack` 回上一屏（主菜单入口回主菜单并重建以同步队伍变化，启动页入口回启动页）。
- **主体双栏（v0.1.29）**：左栏信息框（`.dex-detail`，白底蓝环卡，上 46% 图片区 / 下 54% 文字区）
  展示当前条目的图片（`/images/tool/` 与 `/images/portable Items/` 两级查找，缺图回退首字色块）、
  类型徽章 + 名称、拥有量状态、来源行与效果说明；右栏为可滚动条目列表（`.dex-row` 行卡：
  徽章 + 名称 + 状态），悬停行时环加深并联动左栏（左栏骨架一次构建、只改内容不重建节点，
  杜绝闪烁）；进入页 / 切换筛选后左栏默认展示列表首行。
- **`player` 可为 `null`**（启动页尚未读档）：此时全部条目按「未拥有」只读展示，
  左栏操作行不给穿戴 / 脱下而显示「进入游戏后可在此直接穿戴 / 脱下装备。」，统计卡第三行改示「未载入存档 · 只读浏览」。
- **口径是「全量列出」**：数据来自 `ItemDexData#build(player)`，其条目数与
  `ShopStock#catalog()` 完全相同（当前 **94 件** = 17 件消耗品 + 77 件装备），
  **不做收集解锁**——未拥有的也照常显示全部信息，只在状态位标注「未拥有」。
- **四档筛选**（`ItemDexView.Filter`，单选 `ToggleButton`，`.dex-filter` 胶囊：未选中金色 / 选中蓝芯 + 黄晕）：
  全部 / 道具 / 装备 / 已拥有；切换筛选后列表回到顶部并联动左栏到新首行（换口径重看）；右上统计卡同步给出
  「全量 N 件 / 道具 x 件 · 装备 y 件 / 已拥有 Z 件 · 已穿戴 W 件」。
- **左栏信息框内容**（`.dex-detail`）：类型徽章（道具 = 金徽章 / 装备 = 蓝徽章）+ 名称、拥有量状态（`已拥有 ×N` / `已拥有 · 未穿戴` /
...
- **装备可直接穿脱**（已拥有时，操作行在左栏信息框底部）：未被穿戴 → 「穿给」`ComboBox<Pokemon>`（显示 `名字 Lv.N`，
  默认先发）+「穿戴」按钮；已被穿戴 → 「[名字] 持有」+「脱下」按钮。走的是
  `Player#equip` / `Player#unequip`，因此**全队唯一穿戴**的约束由模型层保证，界面层不重复校验；
  操作后就地刷新统计卡 / 各列表行状态 / 左栏（不重建列表，悬停与滚动位置原样保留）；
  未拥有的装备给出「可以在商店碰运气买到（每次进店随机上架），或在肉鸽『装备补给』事件中获得」提示。
  （启动页入口 `player == null` 时不做穿脱，见上。）
- **消耗品在本页只读**（局外使用入口在主菜单中栏的道具详情框：点「使用」后选左侧闪光目标；本页仍不提供使用按钮）；
  `ItemDexData#describe(Item)` 按类别现推一句效果说明
  （`HEAL` → 回复量、`POKE_BALL` → 捕捉倍率或「必定捕捉成功」、`CURE` → 解除范围、
  `LEVEL_UP` → 「提升精灵 N 级（可在主菜单中栏喂食）」），
...
**`StartView` 入口**（v0.1.22）：启动页在「宝可梦图鉴」下新增第七个胶囊
「道具图鉴」（`ITEM DEX`，行囊图标 `ICON_BAG`，色调 `teal`），构造签名变为
`StartView(onStartGame, onContinueGame, canContinue, onCustomBattle, onPokedex, onItemDex)`；
`MainController#showItemDexFromStart()` 以 `player = null` 打开同一页、返回回启动页。
...
| v0.1.28 | 2026-09-13 | **道具图鉴页视觉统一**：§3.7 `ItemDexView` 重构为与主菜单 / 事件遭遇页同族样式 —— 顶栏一行（返回胶囊 / 标题横幅 / `.rogue-info` 统计卡）、四档筛选胶囊（新增 `.dex-filter`）、卡片改白底蓝环（新增 `.dex-card`，悬停环加深）、类型徽章 + 名称、穿脱下拉开复用 `.slot-action` / `.slot-delete` / `.starter-combo`；新增错峰入场动效（卡片仅前 8 张参与）；筛选切换回顶、穿脱刷新保滚动位置 | [待确认：UI 负责人] |
| v0.1.29 | 2026-09-13 | **道具图鉴改双栏版面**：§3.7 `ItemDexView` 从全宽卡片列表改「左信息框 + 右列表」双栏（45% / 55%）—— 左栏 `.dex-detail`（上 46% 图片区两级查找 / 下 54% 文字区：徽章 + 名称 / 拥有量 / 来源 / 说明 / 穿脱操作行），右栏 `.dex-row` 行卡（悬停环加深 + 联动左栏）；穿脱移入左栏就地刷新；左栏骨架一次构建；主体双栏撑满剩余高度 | [待确认：UI 负责人] |
| v0.1.30 | 2026-09-13 | **商店装备恢复随机上架（每次 3 件）**：§3.6 `ShopView` 段改写 —— 装备分区每次进店从未拥有池（最多 77 件）随机抽 `RouteConfig#SHOP_EQUIPMENT_STOCK_SIZE`（3）件上架（池不足全上），不再全量列出；`ShopStock#allEquipmentEntries` 改回 `#equipmentEntries(segment, random, owned)`（`random` 重新生效、`null` 兜底）、`forSegment` 实现同步；上架顺序「基础价升序 → id」、售价按段通胀、买走即下架不补货；§3.7 图鉴未拥有装备提示同步为「每次进店随机上架」措辞 | [待确认：UI 负责人] |
| v0.1.31 | 2026-09-13 | **商店分区标题瘦身**：§3.6 `ShopView` 货架两个分区标题由「消耗品 · 已解锁 N 件（解锁后一直有货，可重复购买）」/「装备 · 本次上架 N 件（每次进店随机进货，购买后可在主菜单中栏穿戴）」精简为「消耗品」「装备」两行纯文本（`fillStockRows` 不再拼件数与说明文案） | [待确认：UI 负责人] |
| v0.1.32 | 2026-09-13 | **中栏信息框纸张质感**：主菜单中栏简要信息框卡片由纯白底（rgba(255, 255, 255, 0.92)）改为复古纸张质感 —— 浅米黄纵向渐变底 + 中央微光 + 左下/右上两处极淡做旧晕 + 边缘泛黄做旧（多层 radial-gradient）；深蓝描边环保留，投影改暖褐轻影；文字 / 立绘 / 布局 / 其他区域完全不变 | [待确认：UI 负责人] |
