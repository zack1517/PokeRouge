...
> 版本：v0.1.2（草稿） · 日期：2026-09-08 · 模块：JavaFX UI 设计（FXML / Controller / CSS / 界面）
> 角色：UI 负责人 · 状态：**待组长评审**，评审通过前不进入编码
> 依据：《需求文档》§7 界面与交互需求、《测试用例草稿》§D、《接口文档_战斗服务.md》v1.0、
> **本阶段五系统任务分工**（游戏流程&存档 / 宝可梦系统 / 战斗系统 / 肉鸽系统 / JavaFX UI）
> 版本说明：v0.1.2 依据五系统分工重写边界与内部设计（场景调度归流程&存档、界面 FXML 化）；
...
3. **提供页面并响应调度**：向"游戏流程&存档"系统暴露页面注册表，按 `ScreenId`
...
| 系统 | 职责（组长给定） | 与本模块的关系 |
| --- | --- | --- |
| 游戏流程&存档 | 菜单、游戏流程、**场景切换**、保存/读取当前游戏状态 | **调度方**：决定显示哪个页面、传页面参数、处理存档恢复 |
...
| **JavaFX UI（本模块）** | FXML、Controller、CSS、界面 | 消费上述四系统；页面由流程系统调度 |

调用方向：

```
游戏流程&存档 ──调度（ScreenId + 页面参数 / 状态）──► JavaFX UI
     ▲                                                   │ 查询 / 命令
     │                                                   ▼
     └──────────── 场景结果 / 行动请求 ──────────  宝可梦 / 战斗 / 肉鸽 系统
```

- UI 不直接依赖流程系统实现，只依赖**调度契约**（§6.2）；流程系统也不依赖具体页面类，
  只依赖 `ScreenId → 场景工厂` 注册表；
...
├── App.java                                [改造] start()：装配页面注册表并交流程系统启动
...
实际跳转时机由"游戏流程&存档"系统判定（AP 耗尽、战斗结局、读档恢复等）**：
...
```
   ...
   │        └────┬─────┘
   └─────────────┘ 结束 → 回 mainMenu（可存档/读档由流程系统控制）
```

- 战斗页结束后**是否回地图**、失败扣金币还是结束 Run、是否触发奖励页——全部由
  流程系统按结果状态（§4.1 `BattleService.Status`）决策并调度下一页面；
- **存档**：流程系统可保存/恢复任意时刻状态 → 任意页面都可能被直达恢复（§6.3）。

### 3.2 页面总表（对照需求 §7、用例 §D、五系统驱动关系）

| 页面 | 主要功能 | 驱动/数据系统 | 对应用例 |
| --- | --- | --- | --- |
| 主菜单 | 新 Run / 读档 / 退出（退出确认沿用现有 MainController 文案） | 游戏流程&存档 | — |
| 初始选队 | 多选初始宝可梦、数量上限校验、确定出发 | 流程&存档(流程) + 宝可梦(初始池 I-01) | UI-01 |
| 路线地图 | 段节点链、AP 显示、节点可选性（由肉鸽/流程给出判定）、金币/队伍摘要 | 流程&存档 + 肉鸽（I-02） | UI-02 |
...
| 结算 | 失败原因/通关/侵略战结局 + 存档提示 | 游戏流程&存档（I-07） | FLOW-10~17 |
...
| I-02a | Run 状态机调度接口：`startRun(party) / save() / load()` + **场景调度**（§6.2 契约） | 游戏流程&存档 | 地图/页面流转 UI-02 前置；**UI 与流程系统的核心对齐点** |
...
| I-06 | 同类合并 + 性格选择：`Nature` 模型与加成表、合并服务、可选性格列表 | 宝可梦系统 | **范围 [待确认]**（五系统描述未含性格/努力值，若本轮裁剪由组长明示）；UI-04 前置 |
| I-07 | 结算信息：结局枚举（通关/侵略战/战败）+ 存档提示时机 | 游戏流程&存档 | 结算页展示，不自行判定 |
...
### 6.2 场景调度契约（UI 对"游戏流程&存档"的对外接口）[待确认草案]
...
- 页面状态对象（如 `BattlePageState(engine)`、`MapPageState(runContext)`）定义归属：
  [待确认] 由流程系统定义（它负责存档还原）还是各页面自定——建议流程系统统一定义
  便于 `save/load` 序列化；
...
### A. 五系统边界（新增，最高优先）
1. **[功能归属矩阵]** 组长给"需求功能 → 五系统"对应表；重点：AP/路线/节点归流程还是
   肉鸽？医院归谁？性格/努力值/抓捕合并**本轮是否做**、归宝可梦还是肉鸽？
2. **[场景调度]** UI 与流程系统的调度契约定稿（§6.2：`ScreenId`/工厂/`pageState` 形态、
   存档恢复的页面状态体系）——建议评审时流程系统负责人在场；
3. **[主菜单归属]** 流程系统职责含"菜单"：菜单逻辑+存档入口归它无疑，菜单**界面**
   是否仍由本模块实现（本文档按"UI 实现 mainMenu.fxml、流程系统驱动"处理）；
