...
public class RogueFloorView {
     ...
     * device=装备补给 / candy=糖果补给 / exchange=宝可梦交换 / teamrocket=火箭队 /
     * teamrocket_boss=火箭队抓捕神兽 / legendarypokemon=神兽偶遇；均为 2x 396×158 预裁图，
     * 见临时/03-素材与图片处理/event_crop.py）。
    ...
    private static Image eventImageFor(OptionType type) {
        ...
        String name = switch (type) {
            ...
            default -> null; // 必然节点（道馆 / 四天王 / 冠军 / 火箭队入侵）不出现于卡面，无图
        };
        Image image = null;
        if (name != null) {
