...
public class ShopView {
    ...
    /** 本次货架库存；装备购入后会被替换为「去掉该件」的新库存（见 {@link #refresh(ShopStock)}）。 */
    ...
    /**
     * 货架刷新（带新库存）：库存本身发生变化时使用——装备购入后由控制器传入「去掉该件」的新库存，
     * 右栏商品列表按新库存重建，避免同一件装备重复上架。
     *
     * <p>重建只发生在右栏列表内层，仍是同一个 Scene，因此窗口尺寸、背景与左上返回胶囊的入场动画
     * 都不会重播；滚动位置会回到顶部，这与「该商品已下架」的语义一致。</p>
     *
     * @param newStock 新库存；为 {@code null} 时只做一次普通刷新（等同 {@link #refresh()}）
     */
    public void refresh(ShopStock newStock) {
        if (newStock != null) {
            stock = newStock;
        }
        rebuildStockRows();
    ...
    /** 按当前 {@link #stock} 重建右栏商品行；不重建 Scene，仅替换列表内层节点。 */
    private void rebuildStockRows() {
        if (stockListBox == null) {
            return; // 场景尚未构建：createScene 会按最新库存构建
        }
        int gold = session.getRogueRunData().getGold();
        stockRows.clear();
        ...
        // 当前信息框若指向已下架的商品（或原先货架为空），退回第一件，避免左侧停留在不存在的条目上
        boolean stillListed = currentEntry != null && stock.entries().stream()
                .anyMatch(e -> e.itemId().equals(currentEntry.itemId()));
        if (!stillListed) {
            currentEntry = stock.entries().get(0);
        }
    }
