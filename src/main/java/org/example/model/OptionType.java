 ...
 *       固定出现，非常驻节点（商店 / 装备补给 / 宝可梦交换 / 糖果补给 / 火箭队 / 神兽偶遇）按概率出现；</li>
 ...
 * 免费（0 点）；共用「特殊事件槽位」的几类节点各取 0/2 点（神兽偶遇 1 点、火箭队线必然
 * 触发的那次神兽偶遇 0 点、火箭队节点 2 点，见 {@link RouteConfig}）。</p>
 */
public enum OptionType {
    ...
    /** 宝可梦交换：低概率随机事件，行动点 2；提供一只队伍平均等级 +1~2 的宝可梦，可自愿交换（不返还行动点）。 */
    TRADE("宝可梦交换", false, false, 2),
    /** 糖果补给：按所在段获得一批神奇糖果（每段递增，由控制器结算）；低概率出现，行动点 1。 */
    CANDY("糖果补给", false, false, 1),
    /** 火箭队队员：所有时期均可能出现的随机节点，行动点 2；高收益高风险，战败本轮直接结束。 */
    ROCKET("火箭队", false, false, 2),
    /** 火箭队抓捕神兽：后期开启剧情线后的可自主选择事件（末段起固定出现），行动点 2；与首领交手，战败本轮直接结束。 */
